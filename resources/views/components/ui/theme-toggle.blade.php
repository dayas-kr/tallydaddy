<div x-data="{
    ...
    moveFocus(dir) {
        ...
        this.focusIndex = (this.focusIndex + dir + len) % len
        this.current = this.options[this.focusIndex]
        themeManager.setTheme(this.current)
    }
}"
    class="relative z-0 inline-grid grid-cols-3 gap-0.5 rounded-full bg-neutral-950/5 p-0.75 text-neutral-950 dark:bg-white/10 dark:text-white"
    role="radiogroup" @keydown.right.prevent="moveFocus(1)" @keydown.left.prevent="moveFocus(-1)">
    <!-- System -->
    <span role="radio" aria-label="System theme" :aria-checked="current === 'system'"
        :tabindex="focusIndex === 0 ? 0 : -1" @click="select('system', 0)" @focus="focusIndex = 0"
        :data-checked="current === 'system' ? true : null"
        class="rounded-full p-1.5 *:size-7 data-checked:bg-white data-checked:ring-2 data-checked:inset-ring data-checked:ring-neutral-950/10 data-checked:inset-ring-white/10 sm:p-0 dark:data-checked:bg-neutral-700 dark:data-checked:text-white dark:data-checked:ring-transparent cursor-pointer focus:outline-hidden">
        <svg viewBox="0 0 28 28" fill="none">
            ...
            <path
                ...
                stroke="currentColor"></path>
            <path d="M16.5 20.5V17.5H11.5V20.5M16.5 20.5H11.5M16.5 20.5H17.5M11.5 20.5H10.5" stroke="currentColor"
                stroke-linecap="round"></path>
        </svg>
    </span>

    <!-- Light -->
    <span role="radio" aria-label="Light theme" :aria-checked="current === 'light'"
        :tabindex="focusIndex === 1 ? 0 : -1" @click="select('light', 1)" @focus="focusIndex = 1"
        :data-checked="current === 'light' ? true : null"
        class="rounded-full p-1.5 *:size-7 data-checked:bg-white data-checked:ring-2 data-checked:inset-ring data-checked:ring-neutral-950/10 data-checked:inset-ring-white/10 sm:p-0 dark:data-checked:bg-neutral-700 dark:data-checked:text-white dark:data-checked:ring-transparent cursor-pointer focus:outline-hidden">
        <svg viewBox="0 0 28 28" fill="none">
    ...
    <!-- Dark -->
    <span role="radio" aria-label="Dark theme" :aria-checked="current === 'dark'"
        :tabindex="focusIndex === 2 ? 0 : -1" @click="select('dark', 2)" @focus="focusIndex = 2"
        :data-checked="current === 'dark' ? true : null"
        class="rounded-full p-1.5 *:size-7 data-checked:bg-white data-checked:ring-2 data-checked:inset-ring data-checked:ring-neutral-950/10 data-checked:inset-ring-white/10 sm:p-0 dark:data-checked:bg-neutral-700 dark:data-checked:text-white dark:data-checked:ring-transparent cursor-pointer focus:outline-hidden">
        <svg viewBox="0 0 28 28" fill="none">
            <path
